 ...
 * UI does it).
 */
public class OkeraAuthenticator implements PasswordAuthenticator {
  ...
  public OkeraAuthenticator(OkeraConfig serverConfig) {
    ...
    if (envVarSet("SYSTEM_TOKEN") || envVarSet("KERBEROS_KEYTAB_FILE")) {
      ...
      if (!envVarSet("CDAS_REST_SERVER_SERVICE_HOST") &&
          !envVarSet("CEREBRO_REST_FQDN")) {
        throw new IllegalStateException(
            "Expecting CDAS_REST_SERVER_SERVICE_HOST or CEREBRO_REST_FQDN " +
      ...
      try {
        ...
        throw new UncheckedExecutionException(
            "Could not configure REST server connection.", e);
      }
    } else {
  ...
  private Principal authenticate(Credential credentials) {
  ...
  private String authenticateRestServer(String user, String passwordOrToken)
      throws IOException {
    user = user.trim();
    ...
    if (isLikelyToken(passwordOrToken)) {
      ...
      final String basicAuth = "Basic " +
          new String(Base64.getEncoder().encode(
              userpass.getBytes(StandardCharsets.UTF_8)), StandardCharsets.UTF_8);
        ...
        if (conn.getResponseCode() >= 400 && conn.getResponseCode() < 500) {
          ...
          if (conn.getResponseCode() == 401 || conn.getResponseCode() == 403) {
            throw new AccessDeniedException(String.format(
                "Failed to authenticate user. Server returned:%n%s%n" +
    ...
    // We don't use a json library to extract the username because of dependency
    // management.
    ...
    // Verify that the authenticated user matches what was specified. In the case of
    // JWT tokens in particular, they can be mismatched. While we could handle this
    // (have the token one superceded, this confuses presto - the UI shows the specified
    // user for example).
    if (!user.equals(authenticatedUser)) {
      LOG.warn(
          "Authentication error for user [%s]. Specified username did not match.",
          user);
      throw new AccessDeniedException("Authentication error for user: " + user +
          ". If using token based authentication, username must match user in token.");
    }
    return authenticatedUser;
  }

  private static synchronized void configureRestServerSsl()
      throws KeyManagementException, NoSuchAlgorithmException {
    if (restServerSslConfigured) {
      return;
    }

    // Configure the all-trusting cert and ignore host mismatch. This is used
    // for internal (within cluster) communication only.
